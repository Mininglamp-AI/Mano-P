...
| 指标          | Cloud | Cloud+Bash | Local (Mano-P 4B) | Qwen3-VL-Plus |
| ------------- | :---: | :--------: | :---------------: | :-----------: |
| 通过率        | 83.0% |   90.0%    |       47.0%       |     39.0%     |
| 平均步数/任务 | 10.3  |     —      |        7.5        |     11.2      |
| 平均每步耗时  | 9.3s  |     —      |       8.0s        |     10.2s     |

**按难度分层**

| Tier | 任务数 |    Cloud    |    Local    | Qwen3-VL-Plus |
| ---- | :----: | :---------: | :---------: | :-----------: |
...
| 类别               |  Cloud   |  Local   | Qwen3-VL-Plus |
| ------------------ | :------: | :------: | :-----------: |
| 浏览器/网页 (31)   | 28 (90%) | 23 (74%) |   18 (58%)    |
| 模糊描述 (10)      | 8 (80%)  | 3 (30%)  |    3 (30%)    |
| 文件管理 (7)       | 5 (71%)  | 3 (43%)  |    4 (57%)    |
| 微信 (6)           | 5 (83%)  | 2 (33%)  |    2 (33%)    |
| 企微/飞书/钉钉 (6) | 6 (100%) | 2 (33%)  |    3 (50%)    |
| 系统设置 (6)       | 3 (50%)  | 3 (50%)  |    3 (50%)    |
| WPS/Office (5)     | 5 (100%) |  0 (0%)  |    0 (0%)     |
| 无 open 指令 (5)   | 4 (80%)  | 1 (20%)  |    2 (40%)    |
| 备忘/提醒 (4)      | 4 (100%) | 2 (50%)  |    0 (0%)     |
| 系统工具 (3)       | 3 (100%) | 3 (100%) |    1 (33%)    |
| 长链路 (10)        | 8 (80%)  | 2 (20%)  |    2 (20%)    |
| 跨应用 (5)         | 3 (60%)  |  0 (0%)  |    0 (0%)     |

**Cloud + Bash Tool**

云端模式在 mano-cua v1.1.0-beta 中接入 shell 工具后,可借助系统命令处理纯 GUI 难以完成的任务:
...
Cloud+Bash 通过率提升至 **90/100 = 90%**(+7,其中 2 道为上轮系统代理误配导致的假阴性）。

**端侧的核心价值**

- ✅ **更少步数**:Local 平均 7.5 步 vs Cloud 10.3 步 vs Qwen 11.2 步——专用 GUI 微调让操作更直接
- ✅ **更快推理**:每步 8.0s vs Cloud 9.3s vs Qwen 10.2s
- ✅ **完全本地**:截图与任务描述零外发,无网络依赖
- ✅ **轻量部署**:~6.4 GB 内存即可在 MacBook 上运行
- ✅ **小模型胜大模型**:Mano-P 4B(47%)以全本地 4B 模型超越云端通用 VL 大模型 Qwen3-VL-Plus(39%),体现 GUI 专用微调的价值

**当前差距**

端侧 4B 通过率 47% vs 云端 Cloud 83%,主要差距在模糊描述、跨应用工作流、办公套件深度操作——这也是后续迭代的明确方向。Qwen3-VL-Plus 作为云端通用 VL 大模型仅 39%,主要受限于中文输入焦点问题、非浏览器 app 适配不足以及步数超限,说明通用 VL 能力不等于 GUI Agent 能力。Cloud+Bash 模式通过 `osascript`/`sips`/`defaults write` 等命令将通过率推至 **90%**,展示了 GUI + Shell 混合策略的上限。

**测试配置**

- 硬件:MacBook Pro · Apple M5 · 16GB RAM
- 云端模型:Claude Sonnet 4.5(via `mano.mininglamp.com`)
- 端侧模型:Mano-P 1.0-4B(W8A16,MLX)
- Qwen 模型:Qwen3-VL-Plus(via `llm-gateway.mlamp.cn`)
- 任务集:100 道,覆盖浏览器/网页、App 操作、长链路、跨应用、模糊描述
- mano-cua:v1.0.22(Cloud+Bash 使用 v1.1.0-beta)
...
下表展示了 Mano-P 1.0-4B 在 Apple M5 Pro + Cider 推理 SDK 组合下的实际性能基准。以 W8A16（MLX 原生仅权重量化路径，也是 [Cider quantization benchmark](#-推理加速-sdk) 采用的参照系之一）作为 baseline，启用 Cider 的 W8A8 激活量化后，同一输入下 prefill 时间从 2.839s 降到 2.519s，**prefill 约加速 12.7%**。更多数据参考下文 [⚡ 推理加速 SDK](#-推理加速-sdk) 章节。
...
**CUA Benchmark —— Mano-P 4B 运行于 MacBook Pro M5（16GB）**
