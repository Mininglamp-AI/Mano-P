...
**Cider** 是面向 Apple Silicon 的推理加速 SDK，基于 Apple MLX 构建。它实现了 MLX 原生未提供的在线激活量化算子，以自定义 INT matmul kernel 形式作为 MLX 自定义原语对外暴露，支持完整的 lazy evaluation。Cider 同时扩展了 `mlx_vlm`（修复多个 bug 并补充端侧推理服务），兼容任意 MLX 模型（Qwen、Llama、Mistral，以及 Qwen3-VL 等 VLM），通过一行 `convert_model()` 即可完成接入，并内置 OpenAI 兼容的 VLM 推理服务。
...
MLX 原生仅支持 **W4A16** 与 **W8A16**，Cider 补齐了 **W8A8** 与 **W4A8** 两种模式。MLX 的原生量化仅作用于权重（`QuantizedLinear` 将权重反量化为 FP16 后走 FP16 GEMM），Cider 则通过 fused quantize-matmul-dequant 原语实现真正的 INT8 激活量化 + INT8 TensorOps 计算：

- **W8A8**：相对 MLX W4A16 快 1.4x–2.2x（视 batch size 而定）
- **W4A8**：权重内存仅为 W8A8 的一半，在 batch size ≥ 16 时与 MLX W4A16 相当
...
**端到端 VLM Prefill（Qwen3-VL-2B，chunked prefill chunk=2048，bfloat16 模型）**
...
M4 上的 **ANE+GPU 异构张量并行**模式（实验性）在 prefill 阶段将约 65% 的输出通道卸载到 Apple Neural Engine，可额外带来 3%–16% 的加速。
...
| 配置  | 准确率    | 平均步数 | 平均步时 | 平均 Tokens/步 |
| ----- | --------- | -------- | -------- | -------------- |
| W8A16 | **58.0%** | 6.1      | 10.1s    | 3,389          |
| W8A8  | **54.0%** | 6.93     | 10.4s    | 3,104          |

指标说明：_准确率_ —— judge 判定与预期 PASS/FAIL 一致的任务占比；_步数_ —— 单个任务中 agent 执行的动作数；_步时_ —— 单步模型推理耗时（端到端时间更长）；_Tokens/步_ —— prompt 与 generation 的 token 合计。

> **关于此硬件上的 W8A8：** W8A8 行通过 [Cider](https://github.com/Mininglamp-AI/cider) 实现 INT8 激活量化。W8A8 通过 INT8 TensorOps 加速 prefill，但需要同时保留原始权重与 INT8 权重，权重内存近似翻倍。在 16 GB 统一内存设备上，额外的内存压力可能触发换页，抵消 prefill 的加速收益，因此本测试中 W8A8 的准确率与步时反而略差于 W8A16。建议预留超出模型体积 4 GB 以上的空闲内存，才能发挥 W8A8 的完整优势。
