...
### 6. Mano-P 端侧 vs 云端任务执行对比

<details>
<summary>📊 展开评测数据</summary>

在 100 道真机 macOS GUI 任务上(MacBook Pro · Apple M5 · 16GB)对比 Mano-P 1.0-4B 端侧推理 vs Claude Sonnet 4.5 云端推理:
...
| Tier | 任务数 | 说明                                     |
| ---- | :----: | ---------------------------------------- |
| A    |   25   | 单页交互:搜索、点击、表单填写            |
| B    |   45   | 多步操作、文档创建、设置导航、需登录应用 |
| C    |   30   | 长链路、跨应用、无 open 提示、模糊描述   |

**端侧的核心价值**

- ✅ **更少步数**:两端都通过的 41 道任务,端侧平均 4.5 步 vs 云端 7.3 步——直接动手,不需要中间验证截图
- ✅ **更快推理**:每步 8.0s vs 9.3s
- ✅ **完全本地**:截图与任务描述零外发,无网络依赖
- ✅ **轻量部署**:~6.4 GB 内存即可在 MacBook 上运行

**当前差距**

端侧 4B 通过率 47% vs 云端大模型 84%,主要差距在模糊描述、跨应用工作流、办公套件深度操作——这也是后续迭代的明确方向。云端模式在 mano-cua v1.1.0-beta 中接入 shell 工具后,可借助 `osascript`/`sips`/`defaults write` 等命令处理纯 GUI 难以完成的任务,通过率提升至 **90/100**。
